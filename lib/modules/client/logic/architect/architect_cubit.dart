...
class ArchitectCubit extends Cubit<ArchitectState> {
  ...
  Map<String, dynamic>? _selectedArchitectDetails;
  List<Map<String, dynamic>> _portfolios = [];
  List<CertificationModel> _certifications = [];
  List<Map<String, dynamic>> _allPortfolios = [];
  List<Map<String, dynamic>> _collaborationRequests = [];
  List<Map<String, dynamic>> _reviews = [];
  List<Map<String, dynamic>> _portfolioReviews = [];
  Map<String, dynamic> _stats = {};
  ProfileModel? _selfProfile;

  ArchitectCubit({SupabaseClient? supabase})
      : _supabase = supabase ?? Supabase.instance.client,
        super(const ArchitectInitial());

  void _emitLoaded() {
    emit(ArchitectLoaded(
      architects: _architects,
      selectedArchitectDetails: _selectedArchitectDetails,
      portfolios: _portfolios,
      certifications: _certifications,
      allPortfolios: _allPortfolios,
      collaborationRequests: _collaborationRequests,
      reviews: _reviews,
      portfolioReviews: _portfolioReviews,
      stats: _stats,
  ...
  Future<Map<String, dynamic>?> fetchArchitectDetails(String architectId) async {
    try {
      ...
      _selectedArchitectDetails = details;
      _emitLoaded();
      return details;
    } catch (e) {
  ...
  Future<bool> addCertification({
    ...
    try {
      ...
      _isLoading = false;
      await fetchCertifications(userId);
      return true;
    } catch (e) {
  ...
  // =========================================================
  // ALL PORTFOLIOS (FROM ALL ARCHITECTS)
  // =========================================================
  ...
  Future<bool> addPortfolioReview({
    ...
    try {
      ...
      await fetchPortfolioReviews(portfolioId);
      return true;
    } catch (e) {
  ...
  Future<String?> submitArchitectOffer({
    ...
    try {
      ...
      final bidResponse = await _supabase.from('bids').insert({
        ...
        'status': 'pending',
        'estimation_months': (durationDays / 30).ceil(),
      }).select('id').single();
